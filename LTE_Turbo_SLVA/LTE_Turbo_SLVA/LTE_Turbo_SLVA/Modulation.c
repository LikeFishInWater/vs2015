...
void Modulation(int modType, double *mod_symbols_rel, double *mod_symbols_img, int *input ,int length_input)
{
	...
	for (i=0; i<(length_input/modType); i++)
	{
		...
		for (j=0; j<modType; j++)
		{
			tempp += input[modType*i+j]<<(modType-1-j);//假如是8QAM：input[0]*2^3+input[1]*2^2+input[2]*2+input[3]
		}
		mod_symbols_rel[i] = sMap_real[tempp];//为input中的每modType个bits分配一个符号，即调制。调制输出的长度是length_input/modType
...
symb_rel:			星座图中相应点的实部
symb_img:			星座图中相应点的虚部
gauss:				用到其中的gauss->isigma、gauss->sigma
*
* Output:
temp:				用来解调
*
...
********************************************************************************
*/
double trans_pdf(double detected_symb_rel, double detected_symb_img, double symb_rel, double symb_img, double snr)//解调部分用到
{
	double temp;
	double sigma;
	double isigma;
	sigma = sqrt(0.5 / ((4 /3.0) * (pow(10.0, (snr / 10.0)))));
		...
		for (i=0; i<(lenOutput/modType); i++)/////////////共有lenOutput/TC->sMap->m个调制产生的符号
		{
			...
			{				
				...
				if ((j>>(modType-k-1)) % 2 == 1)
				{
					...
					sum1 += trans_pdf(detected_symb_rel[i], detected_symb_img[i], sMap_real[j], sMap_imag[j], snr) * mul1;
				}						
				else
				{
					if (bigIT != 0)
					...
					sum0 += trans_pdf(detected_symb_rel[i], detected_symb_img[i], sMap_real[j], sMap_imag[j], snr) * mul0;
				}
		...
		for (i=0; i<(lenOutput/modType); i++)
		{
			...
			{				
				...
				if ((j>>(modType-k-1)) % 2 == 1)
				{
					...
					temp = log(trans_pdf(detected_symb_rel[i], detected_symb_img[i],  sMap_real[j], sMap_imag[j], snr)) + mul1;
					if (temp > sum1)
					{
						sum1 = temp;
					}
				...
				else
				{
					...
					temp = log(trans_pdf(detected_symb_rel[i], detected_symb_img[i],  sMap_real[j], sMap_imag[j], snr)) + mul0;
					if (temp > sum0)
					{
						sum0 = temp;
					}															
...
input:				需要归一化的数据流
length:				需要归一化的数据流的长度
norm_flag:			归一化方式的选择：1->normalize with average power；2->normalize with average amplitude
