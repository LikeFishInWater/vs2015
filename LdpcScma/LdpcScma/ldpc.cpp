...
void CLDPC::Encode(int  * MsgSeq, int * EncodingSeq)
{
	//置消息位
...
unsigned long CLDPC::CalErrorBits(int  * EncodingSeq, unsigned char *DecodingSeq)
{
...
void CLDPC::ParamInit(int MaxIte,int DecAlg,double MS_a ) {
	maxIteration = MaxIte;
	DecodeAlgorithm = DecAlg;
	MS_alpha = MS_a;
}
...
{
	/*构建一个临时矩阵tempH*/
	unsigned char * tempH = new unsigned char[ChkLen * CodeLen]();
...
bool CLDPC::EncodeCheck(int  * EncodingSeq)
{
	int chk;
...
void CLDPC::Decode(double * LLR, unsigned char *DecodingSeq)
{
	switch (DecodeAlgorithm)
	{
	case 0:
		Decode_BP(LLR, DecodingSeq);
		break;
	case 1:
		Decode_MS(LLR, MS_alpha, DecodingSeq);
		break;
	case 2:
		Decode_BF(LLR, DecodingSeq);
		break;
	default:
		break;
	}
...
void CLDPC::Decode_BP(double * LLR, unsigned char *DecodingSeq)
{
	/*BP LLR*/
	/*计算外信息，对数似然比*/
	/*初始化Q，Q为变量节点的输出消息*/
...
void CLDPC::Decode_MS(double * LLR, double alpha, unsigned char *DecodingSeq)
{
	/*MS LLR*/
	/*初始化Q，Q为变量节点的输出消息*/
	unsigned long i;
...
void CLDPC::Decode_BF(double * LLR, unsigned char *DecodingSeq)
{
	/*BF LLR*/
	unsigned long i;
	unsigned int d;
