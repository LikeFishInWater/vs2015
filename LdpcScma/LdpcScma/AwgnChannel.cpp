...
void CChannel::AWGNChannel(double * inSymbolSeqRe, double * inSymbolSeqIm, double sigma, int SymbolLen)
{
	//	ofstream fout("modsynbol.txt");
	for (unsigned long i = 0; i < SymbolLen; i++)
	{
		//		fout << inSymbolSeq[i].Real << endl;
		inSymbolSeqRe[i] = Random_Norm(sigma, RS) + inSymbolSeqRe[i];
		inSymbolSeqIm[i] = Random_Norm(sigma, RS) + inSymbolSeqIm[i];
	}
	//	fout.close();
}
