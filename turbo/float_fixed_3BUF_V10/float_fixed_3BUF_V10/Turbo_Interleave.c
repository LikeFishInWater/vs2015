...
/*	int cycleBuf[3*6176] = {0};								//cycle buffer
	...
	lenCB = (int)(rateMacthLen/3);

	for(i = 0;i<lenCB;i++)
	{
		rateMatchOut[i] = systemBit[i];
		rateMatchOut[lenCB+i] = parityBit[i];
		rateMatchOut[2*lenCB+i] = iParityBit[i];
	}

}
/*
********************************************************************************
* NAME:				SubInterleave_double
* PURPOSE:			子块交织函数，针对浮点数，用来将译码器计算出来的先验信息交织后赋予demodulation
*
* Input:
inputBit:			输入的待交织的序列
R:					交织矩阵的行数
...
/*
********************************************************************************
* NAME:				TurboInterleave_double
* PURPOSE:			对一个码块进行Turbo速率匹配，针对浮点数，用来将译码器计算出来的先验信息交织后赋予demodulation
*
* Input:
codeBlockSize:		码块的长度
f1:					QPP交织器的参数
...
{
	double cycleBuf[3*6176] = {0};								//cycle buffer
	int cycleBufSize = 0;															//cycle buffer size
	int codeBlockSizeAddTail = codeBlockSize + 4;			//
	int C = 32;																				//column number of Turbo_Interleave matrix
	int interMatrixSize = 0;														//Turbo_Interleave matrix size
	...
	//interleave system bit and save it to cycle buf
	memset(temp, 0, sizeof(double)*R*C);
	SubInterleave_double(systemBit, temp, 0, R, C, F0, P);//offset is 0
	for (i=0; i<R*C; i++)
	{
		cycleBuf[cycleBufSaveAddr] = temp[i];
		cycleBufSaveAddr++;
	...
	//interleave parity bit and save it to cycle buf
	memset(temp, 0, sizeof(double)*R*C);
	SubInterleave_double(parityBit, temp, 0, R, C, F0, P);//offset is 0
	for (i=0; i<R*C; i++)
	{
		cycleBuf[cycleBufSaveAddr] = temp[i];
		cycleBufSaveAddr += 2;
	...
	cycleBufSaveAddr = R*C + 1;
	memset(temp, 0, sizeof(double)*R*C);
	SubInterleave_double(iParityBit, temp, 1, R, C, F0, P);//offset is 1
	for (i=0; i<R*C; i++)
	{
		cycleBuf[cycleBufSaveAddr] = temp[i];
		cycleBufSaveAddr += 2;
	...
	while (rateMatchSaveAddr < rateMacthLen)
	{
		...
		{
			rateMatchOut[rateMatchSaveAddr] = cycleBuf[cycleBufGetAddr];
			//printf("%f\t",rateMatchOut[rateMatchSaveAddr]);
			rateMatchSaveAddr++;
		}
		cycleBufGetAddr = (cycleBufGetAddr+1)%cycleBufSize;
	}
