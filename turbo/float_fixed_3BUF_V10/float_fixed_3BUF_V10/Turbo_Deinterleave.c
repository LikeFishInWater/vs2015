...
*/
#include "define.h"
#include "Turbo_Deinterleave.h"

/*
********************************************************************************
* NAME:				TurboDeinterleave
* PURPOSE:			解交织，用于解Turbo速率匹配中的子块交织
*
* Input:
...
/*	int C = 32;				//row number of interleave matrix
	...
	memset(systemBit, 0, sizeof(double)*(maxInfoLength+4));
	memset(parityBit, 0, sizeof(double)*(maxInfoLength+4));
	memset(iParityBit, 0, sizeof(double)*(maxInfoLength+4));
	getAddr = k0;
	getBitLen = 0;
