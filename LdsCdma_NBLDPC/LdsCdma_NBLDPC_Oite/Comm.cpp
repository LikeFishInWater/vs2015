...
bool CComm::Initial(CSimulation &sim)
{
	...
	RX_LLR_init = new double*[16];
	for (int i = 0; i < 16; i++)
	{
		RX_LLR_BIT[i] = new double[CODE_BIT_LEN];
		RX_LLR_init[i] = new double[CODE_BIT_LEN];
	}
	
	RX_LLR_SYM = new double**[16];
	B_sym_LLR = new double**[16];
	for (int i = 0; i < 16; i++)
	{
		RX_LLR_SYM[i] = new double*[CODE_SYM_LEN];
		B_sym_LLR[i] = new double*[CODE_SYM_LEN];
	}
	for (int i = 0; i < 16; i++)
	{
		for (int j = 0; j < CODE_SYM_LEN; j++)
		{
			...
			B_sym_LLR[i][j] = new double[codeOrder - 1];
		}
	}
...
int CComm::Decode(void)
{
	...
	for (int i = 0; i < 16; i++)
		for (int j = 0; j < CODE_SYM_LEN;j++)
			for (int t = 0; t < codeOrder - 1;t++)
				B_sym_LLR[i][j][t] = 0;
	for (int outite = 0; outite < out_ite; outite++)
	{
		...
		for (int i = 0; i < 16; i++)
		{
			for (int j = 0; j<CODE_SYM_LEN; j++)
				for (int t = 0; t < codeOrder - 1; t++)
				{
					RX_LLR_SYM[i][j][t] = RX_LLR_SYM[i][j][t] - B_sym_LLR[i][j][t];
				}
			DecodeCorrect = NBLDPC.Decoding(RX_LLR_SYM[i], RX_DECODE_SYM[i]);
			for (int j = 0; j<CODE_SYM_LEN; j++)
				for (int t = 0; t < codeOrder - 1; t++)
				{
					B_sym_LLR[i][j][t] = NBLDPC.L_post[j][t] - RX_LLR_SYM[i][j][t];
				};
			//RX_LLR_SYM to RX_LLR_BIT
			for (int j = 0; j < CODE_SYM_LEN; j++)
				for (int t = 0; t < CODE_BIT_PER_SYM; t++)
				{
					One = -1000;
					Zero = 0;
					for (int tt = 1; tt < 16; tt ++ )
					{
						if (LLR_conv[t][tt] == 1)
						{
							if (B_sym_LLR[i][j][tt - 1] > One)
								One = B_sym_LLR[i][j][tt - 1];
						}
						else
						{
							if (B_sym_LLR[i][j][tt - 1] > Zero)
								Zero = B_sym_LLR[i][j][tt - 1];
						}
					}
					RX_LLR_init[i][j*CODE_BIT_PER_SYM + t] = Zero - One;
				}
		}
	}
